...
tabela <- EstabelecimentosESalas %>% 
  ...
  kable(col.names = c("Cidade","Federal (%)","Estadual (%)","Municipal (%)","Particular (%)"),
        caption = paste("Percentual de utilização das salas de aula nas cidades da Microrregião de",LocRef$Microrregiao,"em",max(EstabelecimentosESalas$Ano),"nas diferentes redes de ensino"))
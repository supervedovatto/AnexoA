...
tabela1 <- IDM %>% 
  filter(!is.na(Valor) & Localidade == LocRef$Localidade & Ano == max(Ano) & IDM == "Educação") %>% 
...
tabela2 <- IDM %>% 
  merge(RegioesGoias) %>% 
  filter(!is.na(Valor) & RPSEGPLAN == LocRef$RPSEGPLAN & Ano == max(Ano) & IDM == "Educação") %>% 
...
tabela3 <- IDM %>% 
  filter(!is.na(Valor) & Ano == max(Ano) & IDM == "Educação") %>% 
  group_by(Localidade,Variável) %>% 
  summarise(Valor = mean(Valor,rm.na = TRUE)) %>% 
  group_by(Variável) %>% 
  summarise(Valor = median(Valor,rm.na = TRUE)) %>% 

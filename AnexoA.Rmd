---
title: "Anexo A"
subtitle: "Relatório de Estudo/Pesquisa Natural, Social, Econômica e Educacional dos Municípios e das Regiões de Influência dos Campus do IFG"
author: "Observatório do Mundo do Trabalho - Campus Goiânia"
documentclass: book
date: "`r format(Sys.time(), '%d de %B de %Y')`"
output:
  pdf_document:
header-includes:
- \usepackage[brazil]{babel}
- \usepackage{geometry} 
- \geometry{ a4paper, inner=20mm, top=20mm, outer=20mm, bottom=20mm }
fontsize: 11pt
editor_options: 
  chunk_output_type: console
---

```{r load_packages, include=FALSE, warning=FALSE}
library(tidyverse)
library(reshape2)
library(ggthemes)
library(extrafont)
library(knitr)
library(scales)
library(janitor)
library(kableExtra)
library(ggradar)
library(forcats)
library(lubridate)
```

```{r setup, include=FALSE, warning=FALSE}
knitr::opts_chunk$set(echo = FALSE,
                      fig.align = 'center',
                      warning = FALSE)

rm(list = ls())

mypallete <- c("#3366cc","#dc3912","#ff9900","#109618","#990099","#0099c6","#dd4477","#66aa00","#b82e2e","#316395","#994499","#22AA99","#AAAA11","#6633CC","#E67300","#8B0707","#651067","#329262","#5574A6","#3B3EAC")

options(scipen=10000)

#Para que o relatório seja gerado basta informar a cidade e o ano de referẽncia abaixo.
#Todas as demais estatísticas, gráficos e tabelas serão gerados a partir dessas informações.

#Carrega todos os banco de dados necessários
load(file = "Dados/BDE.RData")

#Para que o relatório seja gerado basta informar a cidade e o ano de referẽncia abaixo.
#Todas as demais estatísticas, gráficos e tabelas serão gerados a partir dessas informações.
Municipio <- "Goiânia"
DataRef <- "2020-01-01"
AnoRef <- lubridate::year(DataRef)

LocRef <- RegioesGoias %>% 
  filter(Localidade == Municipio)

PopulacaoLocal <- PopulacaoProjecao %>%
    filter(Ano == max(PopulacaoProjecao$Ano) & Localidade == LocRef$Localidade) %>%
    summarise(Populacao = sum(Quantidade))

AreaLocal <- Area %>%
    filter(Ano == max(Area$Ano) & Localidade == LocRef$Localidade) %>%
    summarise(Area = sum(AreaTerritorial))

myformat <- function(x) format(x,big.mark=".",decimal.mark=",",scientific=FALSE)

save.image(file = "Dados/POCV.RData")
```

# Apresentação

Nesse relatório fazemos alguns estudos complementares ao que já se encontra disponível nos sites:

  - [Atlas Brasil - Região Metropolitana de Goiânia](http://www.atlasbrasil.org.br/2013/pt/perfil_rm/goiania)
  - [Atlas Brasil - Goiânia](http://www.atlasbrasil.org.br/2013/pt/perfil_m/goiania_go)
  
## O Estudo

O estudo que se segue utiliza como base os dados da plataforma Instituto Mauro Borges.

## Características do Município de `r LocRef$Localicade`

A cidade de `r LocRef$Localidade` possui em `r AnoRef` uma população projetada de `r myformat(PopulacaoLocal)` habitantes e uma área territorial de `r myformat(AreaLocal)` km², deste modo a densidade populacional atual fica projetada em `r myformat(PopulacaoLocal/AreaLocal)` hab/km².
A cidade de `r LocRef$Localidade` está localizada, de acordo com a classificação do IBGE, na *Mesorregião `r LocRef$Mesorregiao`* e na *Microrregião de `r LocRef$Microrregiao`* e segundo a SEGPLAN na *Região de Planejamento `r LocRef$RPSEGPLAN`*.

## Densidade Demográfica

### Mesorregião `r LocRef$Mesorregiao`

```{r}
source(file = "AnaliseMesorregiao.R")
```

A *Mesorregião `r LocRef$Mesorregiao`*, possui em `r AnoRef` uma população projetada de `r myformat(PopulacaoMeso)` habitantes, incluindo as *Microrregiões de `r LocRefMicro$Microrregiao`* compreendendo um total de `r length(LocRefMeso$Localidade)` cidades com uma área total de `r myformat(AreaMeso)` km² sendo as cinco cidades mais populosas `r head(LocRefPop$Localidade)`.
Os municípios menos populosos são `r tail(LocRefPop$Localidade)`.

```{r,fig.cap=paste("Percentual da população em cada Microrregião da Mesorregião",LocRef$Mesorregiao)}
source(file = "GraficoPopulacaoMesorregiao.R")
grafico
```

```{r}
source(file = "TabelaPopulacaoMesoregiao.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Microrregião","População (hab)","Área (km²)","Densidade Populacional (hab/km²)"),
      caption = paste("Densidade Populacional da Mesorregião de",LocRef$Mesorregiao)) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap=paste("Número de municípios em cada Microrregião do",LocRef$Mesorregiao)}
source(file = "GraficoMunicipiosMicrorregioes.R")
grafico
```

### Microrregião de `r LocRef$Microrregiao`

```{r}
source(file = "AnaliseMicrorregiao.R")
```

A *Microrregião de `r LocRef$Microrregiao`*, possui em `r AnoRef` uma população projetada em `r myformat(PopulacaoMicro)` habitantes, definida pelo IBGE, compreende um total `r length(LocRefMicro)` cidades com uma área total de `r myformat(AreaMicro)` km² sendo as cinco cidades mais populosas `r head(LocRef$MicrorregiaoPop$Localidade)`.
Os municípios menos populosos são `r tail(LocRef$MicrorregiaoPop$Localidade)`.

```{r,fig.cap=paste("Percentual da população em cada cidade que a compõem a",LocRef$Mesorregiao)}
source(file = "GraficoPopulacaoMicrorregiao.R")
grafico
```

```{r}
source(file = "TabelaPopulacaoMicrorregiao.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Cidade","População (hab)","Área (km²)","Densidade Populacional (hab/km²)"),
      caption = paste("Densidade Populacional da Microrregião de",LocRef$Microrregiao)) %>% 
kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

### Região de Planejamento `r LocRef$RPSEGPLAN`

```{r}
source(file = "AnaliseRegiaoPlanejamento.R")
```

A *Região de Planejamento `r LocRef$RPSEGPLAN`*, definida pela SEGPLAN, compreende um total `r nrow(LocalSEGPLAN)` cidades sendo as cinco cidades mais populosas `r head(RPSEGPLANPop$Localidade)`.
Os municípios menos populosos são `r tail(RPSEGPLANPop$Localidade)`.
A população da *Região de Planejamento `r LocRef$RPSEGPLAN`* em `r AnoRef` é projetada em `r myformat(PopulacaoSEGPLAN)` habitantes e sua área territorial é `r myformat(AreaSEGPLAN)` km².

```{r}
source(file = "TabelaPopulacaoRPSEGPLAN.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Região de Planejamento","População (hab)","Área (km²)","Densidade Populacional (hab/km²)"),
      caption = "Densidade populacional em cada região de planejamento definida pela SEGPLAN") %>% 
kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap="Percentual da população de Goiás em cada Região de Planejamento definida pela SEGPLAN"}
source(file = "GraficoPopulacaoSEGPLAN.R")
grafico
```

Cidades da Região de Planejamento `r LocRef$RPSEGPLAN`.

```{r}
source(file = "TabelaPopulacaoCidadesRPSEGPLAN.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Cidade","População (hab)","Área (km²)","Densidade Populacional (hab/km²)"),
      caption = paste("Densidade populacional em cada cidade da Região de Planejamento",LocRef$RPSEGPLAN)) %>% 
kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap=paste("Percentual da população em cada cidade da Região de Planejamento",LocRef$RPSEGPLAN)}
source(file = "GraficoPopulacaoCidadesSEGPLAN.R")
grafico
```

# Aspectos Econômicos

## Painel das Profissões

## Caracterização Geral das Profissões no Município

```{r}
source(file = "TabelaEmpregosCAGED.R")
tabela %>% 
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Saldo acumulado da geração de empregos por setor entre",max(Emprego$Ano)-5,"e",max(Emprego$Ano),"na Região de Planejamento",LocRef$RPSEGPLAN,"segundo a CAGED.")) %>%
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "25em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap=paste("Total de postos de trabalho por setor nos últimos anos em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no Município de",LocRef$Localidade,"segundo dados da RAIS."),fig.asp = 1.618}
source(file = "GraficoEmpregosRAIS.R")
grafico
```

```{r,fig.cap=paste("Admitidos e Desligados por setor nos últimos 10 anos na Região de Planejamento",LocRef$RPSEGPLAN,"segundo dados da CAGED."),fig.asp = 1.618}
source(file = "GraficoEmpregosCAGED.R")
grafico
```
Segundo a Relação Anual das Informações Sociais (RAIS) elaborada pelo Ministério do Trabalho e Emprego (MTE), o número de empregos (postos de trabalho) corresponde ao total de vínculos empregatícios ativos, é diferente do número de pessoas empregadas, pois um mesmo indivíduo pode estar ocupando mais de um posto de trabalho na data de referência.
Como vínculo empregatício entende-se a relação de emprego mantida com o empregador durante o ano-base e que se estabelece sempre que ocorrer trabalho remunerado com submissão hierárquica ao empregador e horário pré estabelecido por este.
Esta relação pode ser regida pela Consolidação das Leis do Trabalho (CLT) ou pelo Regime Jurídico Único, no caso de empregado estatutário.
Adicionalmente a RAIS levanta dados sobre vínculos de trabalhador avulso, trabalhador temporário (Lei nº 6.019, de 03/01/74), menor aprendiz, diretor sem vínculo que tenha optado por recolhimento do FGTS e trabalhador com contrato de trabalho por prazo determinado (Lei nº 9.601, de 21/01/98).

O total de empregos a soma do número de empregos nos setores: *`r head(levels(Emprego$Setor),-1)`* e *`r tail(levels(Emprego$Setor),1)`*.

```{r, results='asis'}
source(file = "TabelaEmpregosRAIS-1.R")
tabela %>%
  mutate_all(linebreak) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Total de empregos por setor, entre",max(Emprego$Ano)-4,"e",max(Emprego$Ano),"no Município de",LocRef$Localidade,"segundo a RAIS.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "20em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

O Rendimento médio do Setor no municipio de Goiânia pode ser Verificado na tabela à seguir.
Os dados foram obtidos diretamente da plataforma Mauro Borges.

```{r, results='asis'}
source(file = "TabelaEmpregosRAIS-2.R")
tabela %>%
  mutate_all(linebreak) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Rendimento salarial médio (em reais) dos empregos por setor, entre",max(Emprego$Ano)-4,"e",max(Emprego$Ano),"no Município de",LocRef$Localidade,"segundo a RAIS.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "20em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r, results='asis'}
source(file = "TabelaEmpregosRAIS-3.R")
tabela %>%
  mutate_all(linebreak) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Rendimento salarial médio (em reais) dos empregos por setor, entre",max(Emprego$Ano)-4,"e",max(Emprego$Ano),"na Região de Planejamento de",LocRef$RPSEGPLAN,"segundo a RAIS.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "20em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r, results='asis'}
source(file = "TabelaEmpregosRAIS-4.R")
tabela %>%
  mutate_all(linebreak) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Rendimento salarial médio (em reais) dos empregos por setor, entre",max(Emprego$Ano)-4,"e",max(Emprego$Ano),"em Goiás segundo a RAIS.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "20em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T)
```

```{r, results='asis'}
source(file = "TabelaEmpregosRAIS-4.R")
tabela %>%
  mutate_all(linebreak) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste("Rendimento salarial médio (em reais) dos empregos por setor, entre",max(Emprego$Ano)-4,"e",max(Emprego$Ano),"em Goiás segundo a RAIS.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  column_spec(1, width = "20em") %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap=paste("Comparativo do rendimento salarial médio (em reais) por setor em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 1.618}
source(file = "GraficoRendimentoMedioRAIS.R")
grafico
```

As taxas para o setor da *Administração Pública Direta e Indireta* são muito pequenas quando comparadas com os demais setores devido à estabilidade no serviço público.
```{r,fig.cap=paste("Comparativo da taxa de aumento do número de postos de trabalho por setor em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 1.618}
source(file = "GraficoSaldoRelativoPorVagasCAGEDxRAIS.R")
grafico
```

```{r,fig.cap=paste("Número de postos de trabalho nos subsetores do comércio em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 1.618}
source(file = "GraficoEmpregosRAIS-Comercio.R")
grafico
```

```{r,fig.cap=paste("Número de postos de trabalho no comercio varejista para cada posto no atacado em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 0.618}
source(file = "GraficoEmpregosRAIS-Comercio-2.R")
grafico
```

```{r,fig.cap=paste("Número de postos de trabalho nos subsetores de Serviços em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 1.618}
source(file = "GraficoEmpregosRAIS-Servicos.R")
grafico
```

```{r,fig.cap=paste("Número de postos de trabalho nos subsetores da Indústria de Transformação em Goiás, na Região de Planejamento",LocRef$RPSEGPLAN,"e no município de",LocRef$Localidade,"nos últimos anos."),fig.asp = 1.618}
source(file = "GraficoEmpregosRAIS-Transformacao.R")
grafico
```

### IDM - Educação

## Caracterização  dos Vínculos Empregatícios

# Aspectos Demográficos e Afins

```{r,fig.cap=paste("Percentual da população atendida por serviços de água e esgoto em",LocRef$Localidade)}
source(file = "GraficoAguaEsgoto.R")
grafico
```

A pirâmide etária da cidade de goiânia baseada na população projetada para 2020 é apresentada no gráfico à seguir:

```{r,fig.cap=paste("Pirâmide etária da cidade de",LocRef$Localidade),fig.asp = 1.618}
source(file = "GraficoPiramideEtaria.R")
grafico
```

# Aspectos Educacionais

```{r,fig.cap=paste("Número de estabelescimentos de ensino infantil, fundamental e médio em",LocRef$Localidade,"por rede de ensino nos últimos anos")}
source(file = "GraficoEstabelescimentos.R")
grafico
```

Corresponde ao total de pessoas em atividades docentes em sala de aula no ensino pré-escolar, no ensino fundamental e no ensino médio.
O mesmo docente pode ministrar aulas em redes e municípios diferentes.
Os dados são referentes à `r max(Docentes$Ano)`.
```{r,fig.cap=paste("Número de docentes nas redes municipal, estadual, federal e particular atuando nos ensinos pré-escolar, fundamental e médio em",LocRef$Localidade,"para cada 100 mil habitantes.")}
source(file = "GraficoDocentesHabitantes.R")
grafico
```

```{r,fig.cap=paste("Percentual de utilização das salas de aula disponíveis em",LocRef$Localidade,"nos últimos anos")}
source(file = "GraficoUtilizacaoSalas.R")
grafico
```

```{r}
source(file = "TabelaUtilizacaoSalasProporcao.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Cidade","Federal (%)","Estadual (%)","Municipal (%)","Particular (%)"),
      caption = paste("Percentual de utilização das salas de aula nas cidades da Região de Planejamento",LocRef$RPSEGPLAN,"em",max(EstabelecimentosESalas$Ano),"nas diferentes redes de ensino")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r}
source(file = "TabelaSalasExistentes.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      col.names = c("Município","Federal","Estadual","Municipal","Particular","Total"),
      caption = paste("Salas de aulas disponíveis nos municípios na Região de Planejamento de",LocRef$RPSEGPLAN,"em",max(EstabelecimentosESalas$Ano))) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.",footnote_as_chunk = T,threeparttable = T)
```

```{r,fig.cap=paste("Evolução do IDEB em",LocRef$Localidade,"nos últimos anos")}
source(file = "GraficoIDEB.R")
grafico
```

```{r,fig.cap=paste("Distribuição do IDEB",lubridate::year(max(IDEB$Ano)),"na Região de Planejamento",LocRef$RPSEGPLAN,"e em Goiás."),fig.asp = 1.618}
source(file = "GraficoIDEB-2.R")
grafico
```

Outras informações sobre o IDM podem ser obtidas pelo link:

 - [IDM 2018](https://www.imb.go.gov.br/files/docs/publicacoes/idm/idm2018.pdf)
 - [IDM 2016](https://www.imb.go.gov.br/files/docs/publicacoes/idm/idm2016.pdf)
 
```{r}
source(file = "GraficoIDM-composicao.R")
source(file = "GraficoIDM-radar.R")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Educação para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Educação")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Educação para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Educação")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM do Trabalho para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Trabalho")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Trabalho para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Trabalho")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Economia para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Economia")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Economia para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Economia")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Infraestrutura para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Infraestrutura")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Infraestrutura para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Infraestrutura")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Saúde para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Saúde")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Infraestrutura para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Saúde")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Segurança para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 2*0.618}
ComposicaoIDM("Segurança")
```

```{r,fig.cap=paste("Participação das diferentes variáveis que compôem o índice do IDM da Segurança para o município de",LocRef$Localidade,"nos últimos anos"),fig.asp = 1.618}
RadarIDM("Segurança")
```

```{r,fig.cap=paste("Distribuição por setor do IDM",lubridate::year(max(IDM$Ano)),"na Região de Planejamento",LocRef$RPSEGPLAN,"e em Goiás."),fig.asp = 1.618}
source(file = "GraficoIDM-distribuição.R")
grafico
```

```{r,fig.cap=paste("IDM",lubridate::year(max(IDM$Ano)),"no Município de",LocRef$Localidade," e IDM mediano na Região de Planejamento",LocRef$RPSEGPLAN,"e em Goiás."),fig.asp = 1.618}
source(file = "GraficoIDM-geral-radar.R")
grafico
```

```{r}
source(file = "TabelaIDM-radar.R")
kable(x = tabela,
      format = "latex",
      booktabs = TRUE,
      caption = paste("IDMs",lubridate::year(max(IDM$Ano)),"no Município de",LocRef$Localidade,"e IDMs médios dos nunicípios da Região de Planejamento",LocRef$RPSEGPLAN,"e de Goiás.")) %>% 
  kable_styling(latex_options = c("striped", "hold_position")) %>% 
  row_spec(0, angle = 90) %>% 
  footnote(general_title = "Fonte:", general = "Elaborado pelo núcleo de base do OMT/GYN a partir de dados do BDE/IMB, com acesso em 19/03/2020.", footnote_as_chunk = T, threeparttable = T)
```

## Dados Institucionais: Atuação do Câmpus no seu território

### Ações de Ensino

### Ações de Pesquisa

### Ações de Extensão

## Dados da Pesquisa Institucional de Acompanhamento de Egressos

# Pesquisa de Campo

# Arranjos Produtivos, Sociais e Culturais

# Escuta da Comunidade

# Considerações Finais

# Referências Bibliográficas
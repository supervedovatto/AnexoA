...
TablePopulacaoMesorregiao
```

```{r,fig.cap=paste("Número de municípios em cada Microrregião do",LocRef$Mesorregiao)}
source(file = "Barplots/MunicipiosMicrorregioes.R")
BarplotMunicipiosMicrorregiao
```
...
```{r}
source(file = "Tables/PopulacaoMicrorregiao.R",print.eval = TRUE)
```
...
```{r}
source(file = "Tables/EmpregosCAGED.R",print.eval = TRUE)
```
